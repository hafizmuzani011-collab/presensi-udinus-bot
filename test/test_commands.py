...
class TestJadwal:
    ...
    async def test_jadwal_update(self, mock_send, monkeypatch):
        ...
        monkeypatch.setattr("handlers._update_schedules_func", _mock)
        with patch("handlers.send_message", mock_send):
            from bot import handle_command
...
class TestTugas:
    ...
    async def test_cek_tugas(self, mock_send, mock_scrape, monkeypatch):
        monkeypatch.setattr("handlers._login_kulino_func", mock_scrape)
        monkeypatch.setattr("handlers.process_and_remind_deadlines", AsyncMock(return_value=None))
        with patch("handlers.send_message", mock_send):
            from bot import handle_command
...
class TestPresensi:
    ...
    async def test_presensi_saya(self, mock_send, mock_presensi, mock_holiday, monkeypatch):
        monkeypatch.setattr("handlers._do_presensi_func", mock_presensi)
...
class TestAutopilot:
    ...
    async def test_autopilot_off(self, mock_send, monkeypatch):
        monkeypatch.setattr("handlers._set_autopilot_func", MagicMock())
        with patch("handlers.send_message", mock_send):
            from bot import handle_command
